...
namespace UserTracker.Tests.States
{
    public class GameStateTests : IDisposable
    {
        ...
        [Fact]
        public void MergeUsers_UpsertsAndBackfillsId()
        {
            var merged = GameState.MergeUsers(new Dictionary<string, ScreepsUser>
            {
                ["u1"] = new ScreepsUser
                {
                    Username = "one",
        ...
        [Fact]
        public void MergeUsers_SkipsInvalidEntries()
        {
            var merged = GameState.MergeUsers(new Dictionary<string, ScreepsUser>
            {
                [""] = new ScreepsUser { Username = "invalid-id" },
                ["u1"] = new ScreepsUser { Username = "" },
                ["u2"] = new ScreepsUser { Username = "ok" }
            });
