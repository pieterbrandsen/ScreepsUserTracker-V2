...
namespace UserTrackerShared.DBClients
{
    public class TimeScaleBatchClient : IDisposable
    {
        ...
        public TimeScaleBatchClient(string host, int port, int batchSize = 1000)
        {
            _host = host;
            _port = port;
    ...
    public static class TimeScaleDBClientWriter
    {
        ...
        private static TimeScaleBatchClient _client = new TimeScaleBatchClient(ConfigSettingsState.TimeScaleDbHost, ConfigSettingsState.TimeScaleDbPort);
    ...
    public static class TimeScaleDBClientState
    {
        ...
        public static async Task WriteScreepsRoomHistory(string shard, string room, long tick, long timestamp, ScreepsRoomHistoryDto screepsRoomHistory)
        {
            try
            {
                ...
                TimeScaleDBClientWriter.UploadData($"history.{ConfigSettingsState.ServerName}.data.", shard, room, timestamp, username, screepsRoomHistory);
            }
            catch (Exception e)
            {
        ...
        public static void WritePerformanceData(PerformanceClassDto PerformanceClassDto)
        {
            try
            {
                TimeScaleDBClientWriter.UploadData($"history.{ConfigSettingsState.ServerName}.performance.{PerformanceClassDto.Shard}.", PerformanceClassDto, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
            }
            catch (Exception e)
            {
        ...
        public static void WriteLeaderboardData(SeaonListItem seasonItem)
        {
            try
            {
                ...
                TimeScaleDBClientWriter.UploadData($"history.{ConfigSettingsState.ServerName}.leaderboard.{seasonItem.Type}.{seasonItem.UserName}.", seasonItem, timestamp);
            }
            catch (Exception e)
            {
                _logger.Error(e, $"Error uploading performance data");
        ...
        public static void WriteSingleUserData(ScreepsUser user)
        {
            try
            {
                TimeScaleDBClientWriter.UploadData($"history.{ConfigSettingsState.ServerName}.users.{user.Username}.", user, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
            }
            catch (Exception e)
            {
                _logger.Error(e, $"Error uploading performance data");
        ...
        public static void WriteAdminUtilsData(AdminUtilsDto data)
        {
            try
            {
                TimeScaleDBClientWriter.UploadData($"history.{ConfigSettingsState.ServerName}.adminutils.", data, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
            }
            catch (Exception e)
            {
                _logger.Error(e, $"Error uploading performance data");
